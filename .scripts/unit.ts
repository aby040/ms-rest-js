...
function runNodeJsUnitTests(): number {
  ...
  return executeSync(`nyc mocha`, repositoryRootFolderPath).status || 0;
}
...
function runBrowserUnitTests(): number {
  ...
  return executeSync(`${mochaChromeFilePath} http://localhost:${portNumber} --timeout 60000`, repositoryRootFolderPath).status || 0;
}
